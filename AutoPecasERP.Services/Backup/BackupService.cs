...
public class BackupService{
 public async Task<string> CriarAsync(string destino){
  Directory.CreateDirectory(destino);var arq=Path.Combine(destino,$"AutoPecasERP_{DateTime.Now:yyyyMMdd_HHmmss}.db");
  await using var db=new ErpDbContext();var source=(SqliteConnection)db.Database.GetDbConnection();await source.OpenAsync();
  await using var target=new SqliteConnection($"Data Source={arq}");await target.OpenAsync();source.BackupDatabase(target);return arq;
 }
